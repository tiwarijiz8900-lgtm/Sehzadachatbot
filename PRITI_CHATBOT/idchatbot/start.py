...
from PRITI_CHATBOT.idchatbot.helpers import is_owner
...
import time
import io
from PRITI_CHATBOT import CLONE_OWNERS, db, PRITI_CHATBOT
...
from PRITI_CHATBOT.idchatbot.helpers import (
    START,
    START_BOT,
    PNG_BTN,
    CLOSE_BTN,
...
AUTO = True
ADD_INTERVAL = 200
users = "PritiChatBot"  # don't change because it is connected from client to use chatbot API key
async def add_bot_to_chats():
    try:
        
        bot = await PRITI_CHATBOT.get_users(users)
        bot_id = bot.id
