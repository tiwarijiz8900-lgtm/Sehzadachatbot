...
from PRITI_CHATBOT import PRITI_CHATBOT, mongo
...
from PRITI_CHATBOT.modules.helpers import (
    START,
    START_BOT,
    PNG_BTN,
    CLOSE_BTN,
...
@PRITI_CHATBOT.on_message(filters.new_chat_members)
        ...
        for member in message.new_chat_members:
            ...
            if member.id == PRITI_CHATBOT.id:
                try:
                    ...
                    pass
                try:
                    invitelink = await PRITI_CHATBOT.export_chat_invite_link(message.chat.id)
                ...
                try:
                    groups_photo = await PRITI_CHATBOT.download_media(
                ...
                count = await PRITI_CHATBOT.get_chat_members_count(chat.id)
                chats = len(await get_served_chats())
                ...
                try:
                    ...
                    if OWNER:
                        await PRITI_CHATBOT.send_photo(
                            int(OWNER_ID),
                            photo=chat_photo,
                            caption=msg,
...
@PRITI_CHATBOT.on_cmd(["start", "aistart"])
async def start(_, m: Message):
    ...
    if m.chat.type == ChatType.PRIVATE:
        ...
        if m.chat.photo:
            try:
                userss_photo = await PRITI_CHATBOT.download_media(m.chat.photo.big_file_id)
                await umm.delete()
                if userss_photo:
        ...
        await m.reply_photo(photo=chat_photo, caption=START.format(PRITI_CHATBOT.mention or "can't mention", users, chats, UP), reply_markup=InlineKeyboardMarkup(START_BOT))
        await add_served_user(m.chat.id)
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton(f"{m.chat.first_name}", user_id=m.chat.id)]])
        await PRITI_CHATBOT.send_photo(int(OWNER_ID), photo=chat_photo, caption=f"{m.from_user.mention} ʜᴀs sᴛᴀʀᴛᴇᴅ ʙᴏᴛ. \n\n**ɴᴀᴍᴇ :** {m.chat.first_name}\n**ᴜsᴇʀɴᴀᴍᴇ :** @{m.chat.username}\n**ɪᴅ :** {m.chat.id}\n\n**ᴛᴏᴛᴀʟ ᴜsᴇʀs :** {users}", reply_markup=keyboard)
...
@PRITI_CHATBOT.on_cmd("help")
async def help(client: PRITI_CHATBOT, m: Message):
    if m.chat.type == ChatType.PRIVATE:
...
async def ping(_, message: Message):
    ...
    await loda.edit_text(
        text=f"нey вαву!!\n{PRITI_CHATBOT.name} ᴄʜᴀᴛʙᴏᴛ ιѕ alιve  αnd worĸιng ғιne wιтн a pιng oғ\n\n**➥** `{ms}` ms\n**➲ ᴄᴘᴜ:** {CPU}\n**➲ ʀᴀᴍ:** {RAM}\n**➲ ᴅɪsᴋ:** {DISK}\n**➲ ᴜᴘᴛɪᴍᴇ »** {UP}\n\n<b>||** ⋆ʟᴏᴠᴇ ᴡɪᴛʜ⋆ [ ꯭𓆰꯭꯭ꜱʜᴇʜᴢᴀᴅᴀ𓆪꯭🝐](https://t.me/ll_WTF_SHEZADA_ll) **||</b>",
...
@PRITI_CHATBOT.on_message(filters.command("stats"))
                ...
                for chat in chats:
                    ...
                    try:
                        if broadcast_type == "reply":
                            m = await PRITI_CHATBOT.forward_messages(
                                chat_id, message.chat.id, [broadcast_content.id]
                            )
                        else:
                            m = await PRITI_CHATBOT.send_message(
